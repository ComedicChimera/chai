...
func (c *Compiler) generateLLVMIRModules() {
	...
	// 	go func(ctx *llvm.Context, pkg *depm.ChaiPackage) {
	// 		defer wg.Done()
...
func (c *Compiler) generateASMOrObjectFiles() []string {
	...
	// stageOutputExt := ".o"
	// codegenFileType := llvm.ObjectFile
	...
	// 		codegenFileType = llvm.AssemblyFile
	// 		stageOutputExt = ".s"
	// 	}
	...
	// triple := llvm.HostTriple()
	// target, ok := llvm.GetTargetFromTriple(triple)
	// if !ok {
	// 	report.ReportFatal("unable to find code generator target: %s", triple)
	// }

	// // Create the target machine for code generation.
	// gctx := llvm.NewContext()
	// defer gctx.Dispose()

	// tm := gctx.NewHostMachine(target, llvm.CodeGenLevelDefault, llvm.RelocDefault, llvm.CodeModelDefault)
